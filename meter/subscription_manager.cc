...
SubscriptionResults generate_sub_results(const Evaluator& evaluator,
  ...
  using interpreter::QueryIndex;
  using interpreter::TagsValuePair;
  SubscriptionResults result;

  std::vector<std::shared_ptr<Query>> queries;
  std::transform(subs.begin(), subs.end(), std::back_inserter(queries),
                 [&evaluator](const Subscription& subscription) {
                   return evaluator.get_query(subscription.expression);
                 });
  //  auto queryIndex = QueryIndex<std::shared_ptr<Query>>::Build(queries);
